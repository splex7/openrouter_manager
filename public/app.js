...
function auditLabel(action) {
  const labels = { "account.bootstrap": "Master 계정 생성", "account.create": "계정 생성", "account.create_auto": "학생 계정 자동 생성", "account.create_bulk": "학생 계정 일괄 생성", "account.update": "계정 정보 수정", "account.password_reset": "초기 비밀번호 재설정", "account.status_update": "계정 상태 변경", "access_policy.update": "관리자 키 권한 변경", "auth.login": "로그인", "auth.login_failed": "로그인 실패", "auth.logout": "로그아웃", "auth.password_change": "비밀번호 변경", "student.create": "학생 생성", "student.update": "학생 수정", "student.deactivate": "학생 비활성화", "student.hard_delete": "학생 완전 삭제", "team.create": "조 생성", "team.update": "조 수정", "team.deactivate": "조 비활성화", "team_member.assign": "조원 배정", "team_member.remove": "조원 해제", "quota.update": "개인 한도 정책 변경", "model_policy.update": "허용 모델 정책 변경", "analytics.sync": "사용량 동기화", "credential.issue": "키 발급", "credential.issue_bulk": "키 일괄 발급", "credential.reissue": "키 재발급", "credential.reveal": "키 조회", "credential.test": "키 연결 테스트", "credential.revoke": "키 폐기", "credential.revoke_bulk": "키 일괄 폐기", "credential.normalize_personal_labels": "기존 개인 키 이름 정리" };
...
function render() {
  ...
  const canManage = Boolean(state.me?.canManageCredentials);
  const canManageAccounts = state.me?.role === "master";
  $("#primaryAction").hidden = ["dashboard", "audits", "accessPolicy"].includes(state.view) || (state.view === "credentials" && !canManage) || (state.view === "accounts" && !canManageAccounts) || (state.view === "modelPolicy" && state.modelPolicy?.restrictionMode === "blocklist"); $("#summary").hidden = state.view !== "dashboard"; $("#managerOffers").hidden = state.view !== "dashboard"; $("#dataPanel").hidden = state.view === "dashboard"; $("#dataPanel").classList.toggle("owner-table-view", state.view === "students"); if (state.view !== "audits") $("#auditMoreButton").hidden = true;
...
async function showKey(key, title, subjectType = null, revoked = false) { if (!state.modelPolicy && state.me?.role === "master") { try { state.modelPolicy = (await api("/api/model-policy")).data; } catch {} } if (state.me?.role === "student") { state.portalRevealSubjectType = subjectType; renderPortalKeyReveal(subjectType); } else { $("#keyRevealTitle").textContent = title; } $("#keyRevokedWarning").hidden = !revoked; $("#keyRevokedWarning").textContent = revoked ? portalKeyRevealText("이 키는 폐기되었습니다. 더 이상 사용하지 말고, 필요한 경우 운영자에게 새 키를 요청하세요.", "This key has been revoked. Do not use it; request a new key from an administrator if needed.") : ""; $("#revealedKey").value = key; $("#connectionJson").value = JSON.stringify({ base_url: $("#openRouterBaseUrl").value, api_key: key }, null, 2); renderKeyTestTools(); setKeyTab("connection"); dialog("keyRevealDialog"); }
